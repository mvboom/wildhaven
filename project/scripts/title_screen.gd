...
##
## THE TUTORIAL BUTTON IS GONE (2026-09-08), and Help is where its content went. Both routed
## to the SAME `coming_soon_screen.gd` placeholder, differing only in the word on the label;
## `future.md` had already deferred the standalone Tutorial screen past v1 on the grounds that
## "v1's first-time nudge covers minimum onboarding on its own", and gdd.md's own title-screen
## paragraph lists New Game / Load Game primary with Settings, Help and Credits secondary —
## no Tutorial. Removing it makes this screen match that paragraph instead of diverging from
## it, and the stuck-player content it promised now lives in `HelpContent`'s own "if nobody has
## moved in yet" section. Settings/Credits (2026-08-25) route to that same page
## chrome and carry real content — the `SettingsOverlay`/`CreditsScreen` controls that
