extends QATestCase
## Check — required-animations check on the Alpaca model.
##
...
const MODEL_PATH: String = "res://assets/animals/alpaca/Alpaca.tscn"
...
func _init() -> void:
	...
	var alpaca: Node = packed.instantiate()
	if not check(alpaca != null, "Alpaca.tscn instantiates"):
		finish()
		return

	var player: AnimationPlayer = _find_animation_player(alpaca)
	...
	print("  AnimationPlayer at %s exposes %d clip(s):" % [
		alpaca.get_path_to(player), clips.size()
	])
