extends QATestCase
## Import check — Punk (one of 5 Human look variants).

const MODEL_PATH: String = "res://assets/animals/human_punk/Punk.tscn"
...
func _init() -> void:
	...
	if not check(inst != null, "Punk.tscn instantiates"):
		finish()
		return
