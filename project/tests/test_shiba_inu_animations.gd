extends QATestCase
## Check — required-animations check on the Shiba Inu model (id/directory `shiba_inu`).
##
...
const MODEL_PATH: String = "res://assets/animals/shiba_inu/ShibaInu.tscn"

## Animations the pipeline requires of every species.
const REQUIRED_CLIPS: PackedStringArray = ["Idle", "Walk"]

## Expected clip count from the tech-art audit. (Canids in this pack have one fewer clip
## than the hooved animals — a single "Attack" rather than Attack_Headbutt + Attack_Kick.)
const EXPECTED_CLIP_COUNT: int = 12


func _init() -> void:
	begin("Shiba Inu animations")
	...
	var shiba: Node = packed.instantiate()
	if not check(shiba != null, "ShibaInu.tscn instantiates"):
		finish()
		return

	var player: AnimationPlayer = _find_animation_player(shiba)
	...
	print("  AnimationPlayer at %s exposes %d clip(s):" % [
		shiba.get_path_to(player), clips.size()
	])
