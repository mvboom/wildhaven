extends QATestCase
## Import check — HoodieCharacter (one of 5 Human look variants).

const MODEL_PATH: String = "res://assets/animals/human_hoodie/HoodieCharacter.tscn"
const REQUIRED_CLIPS: PackedStringArray = ["CharacterArmature|Idle", "CharacterArmature|Walk"]
const EXPECTED_CLIP_COUNT: int = 24


func _init() -> void:
	begin("HoodieCharacter import")
	...
	if not check(inst != null, "HoodieCharacter.tscn instantiates"):
		finish()
		return
