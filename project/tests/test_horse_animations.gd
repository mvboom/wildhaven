extends QATestCase
## Check — required-animations check on the Horse model.
##
...
const MODEL_PATH: String = "res://assets/animals/horse/Horse.tscn"
...
func _init() -> void:
	...
	var horse: Node = packed.instantiate()
	if not check(horse != null, "Horse.tscn instantiates"):
		finish()
		return

	var player: AnimationPlayer = _find_animation_player(horse)
	...
	print("  AnimationPlayer at %s exposes %d clip(s):" % [
		horse.get_path_to(player), clips.size()
	])
