extends QATestCase
## Check — required-animations check on the Cow model.
##
...
const MODEL_PATH: String = "res://assets/animals/cow/Cow.tscn"
...
func _init() -> void:
	...
	var cow: Node = packed.instantiate()
	if not check(cow != null, "Cow.tscn instantiates"):
		finish()
		return

	var player: AnimationPlayer = _find_animation_player(cow)
	...
	print("  AnimationPlayer at %s exposes %d clip(s):" % [
		cow.get_path_to(player), clips.size()
	])
