extends QATestCase
## Required-animations check on the Sheep model. Generated by scripts/asset_pipeline.py.

const MODEL_PATH: String = "res://assets/animals/sheep/Sheep.tscn"
const REQUIRED_CLIPS: PackedStringArray = ["Idle", "Walk"]
const EXPECTED_CLIP_COUNT: int = 6

func _init() -> void:
	begin("Sheep animations")
	var packed: PackedScene = load(MODEL_PATH) as PackedScene
	if not check(packed != null, "%s loads as PackedScene" % MODEL_PATH):
		finish(); return
	var inst: Node = packed.instantiate()
	if not check(inst != null, "Sheep.tscn instantiates"):
		finish(); return
