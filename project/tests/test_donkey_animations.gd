extends QATestCase
## Check — required-animations check on the Donkey model.
##
...
const MODEL_PATH: String = "res://assets/animals/donkey/Donkey.tscn"
...
func _init() -> void:
	...
	var donkey: Node = packed.instantiate()
	if not check(donkey != null, "Donkey.tscn instantiates"):
		finish()
		return

	var player: AnimationPlayer = _find_animation_player(donkey)
	...
	print("  AnimationPlayer at %s exposes %d clip(s):" % [
		donkey.get_path_to(player), clips.size()
	])
