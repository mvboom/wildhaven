extends QATestCase
## Check 2 — required-animations check on the Fox model.
##
...
func _init() -> void:
	...
	var fox: Node = packed.instantiate()
	if not check(fox != null, "Fox.tscn instantiates"):
		finish()
		return

	var player: AnimationPlayer = _find_animation_player(fox)
	...
	print("  AnimationPlayer at %s exposes %d clip(s):" % [
		fox.get_path_to(player), clips.size()
	])
