extends QATestCase
## Check — required-animations check on the Deer model.
##
...
const MODEL_PATH: String = "res://assets/animals/deer/Deer.tscn"
...
func _init() -> void:
	...
	var deer: Node = packed.instantiate()
	if not check(deer != null, "Deer.tscn instantiates"):
		finish()
		return

	var player: AnimationPlayer = _find_animation_player(deer)
	...
	print("  AnimationPlayer at %s exposes %d clip(s):" % [
		deer.get_path_to(player), clips.size()
	])
