extends QATestCase
## Import check — Adventurer (one of 5 Human look variants; the recommended default).
...
func _init() -> void:
	...
	if not check(inst != null, "Adventurer.tscn instantiates"):
		finish()
		return
