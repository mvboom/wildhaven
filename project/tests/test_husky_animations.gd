extends QATestCase
## Check — required-animations check on the Husky model.
##
...
const MODEL_PATH: String = "res://assets/animals/husky/Husky.tscn"
...
func _init() -> void:
	...
	var husky: Node = packed.instantiate()
	if not check(husky != null, "Husky.tscn instantiates"):
		finish()
		return

	var player: AnimationPlayer = _find_animation_player(husky)
	...
	print("  AnimationPlayer at %s exposes %d clip(s):" % [
		husky.get_path_to(player), clips.size()
	])
