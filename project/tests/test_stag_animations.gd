extends QATestCase
## Check — required-animations check on the Stag model.
##
...
const MODEL_PATH: String = "res://assets/animals/stag/Stag.tscn"
...
func _init() -> void:
	...
	var stag: Node = packed.instantiate()
	if not check(stag != null, "Stag.tscn instantiates"):
		finish()
		return

	var player: AnimationPlayer = _find_animation_player(stag)
	...
	print("  AnimationPlayer at %s exposes %d clip(s):" % [
		stag.get_path_to(player), clips.size()
	])
