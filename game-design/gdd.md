...
2. Each tile silently carries the habitat tags its own terrain emits (`water`, `forest`, `open_grass`, `rocks`, …); a neighbourhood's character is the *mix* of tags in an area, not tags bleeding between tiles.
...
2. **~0:03 — The first News Report fires** (the first-time nudge), Read-Aloud available. Its copy names terrain the floor roster reads — cultivated (villager, rabbit) and grass — never terrain no shipped species responds to (#12). **Note since D-52:** rock is no longer the right example, because no shipped species needs `cover` any more; rock still matters for `rocks` (Donkey, Alpaca, Shiba Inu). Dismissable; the Hints toggle disables it forever.
...
5. **~0:40 — The starting stockpile invites a build.** Wood in hand covers exactly the nudge's suggestion (Economy): a small cultivated field, or the House with its confirmation chime.
...
- **The capacity formula (habitat tiers, 2026-09-04 → D-52).** A species carries an ordered list of **tiers**, each its own way to qualify with its own population cap. Capacity is the best qualifying tier:

  `capacity(h, S) = max over tiers T of tier_capacity(h, S, T)`

  Within a tier: a violated **limit** returns 0 outright; a **gate-only** need (divisor 0) returns 0 if absent and contributes no cap if present; every scaling need contributes `floor(count / need.tiles_per_individual)`, and the tier's own `max_individuals` caps the result. **Liebig's law of the minimum survives intact inside each tier** — the scarcest need still caps the population, which is the part that actually teaches something. **There is no lower clamp: capacity can be 0, and 0 means unsuitable.**

  Each **need** carries its own radius and its own divisor, which is what lets one recipe say "a stable right here, and a wide tract of grass". The worked example: a barn and some grass gets a *pair* of horses; a stable, a wide tract and water gets a *herd*, with water binding the herd size — so digging more pond visibly buys more horses.
- **Exclusion limits** — *"at most N of tag X within radius R"*. Limits **gate, they never scale**: a violated limit zeroes its whole tier. This is what makes wild animals want genuinely wild land rather than merely a different quantity of the same land, and it is why every wild species carries a `built` limit. Real habitat science models human disturbance as a suitability factor, so this is not a game-ism.
- **Residents emit tags.** A species may declare `emits_tags`, and its residents contribute those tags to the tile they live on — **counted per individual, not per home tile**, so a house holding four villagers reads as `people = 4`. This makes `people` and `deer` ordinary habitat tags rather than a second mechanic: a dog needs an actual person rather than an empty house, and Stag gates on `deer/4` — four deer support one stag, so a stag cannot appear until a real deer population already lives there. The species emission graph **must stay acyclic**, or capacity oscillates forever across the dirty queue; `HabitatGraph.find_cycle()` enforces it (self-emission is a population threshold, not a cycle).
- **Group arrivals.** A tier carries `arrival_group_size`, so deer arrive as a small group and a villager family arrives together. Landing is **partial, not all-or-nothing**: a group of three into room for two lands two, re-checking capacity per individual.
...
- **Tuning lives per need and per tier**, not as three per-species constants: `HabitatNeed` carries `radius` and `tiles_per_individual`, `HabitatLimit` carries `radius` and `max_count`, and `HabitatTier` carries `max_individuals` and `arrival_group_size`. `scout_radius` remains the fallback a need uses when it declares no radius of its own. All are proposed from ecology and **decided by the human** (#6, #20, #23) — the shipped values are proposals awaiting sign-off. Per-need radius band is **2–16** (ruled 2026-09-04, replacing ~8–12). Full per-species table: [roster.md](roster.md).
- **Habitat tag vocabulary** (extended 2026-09-04; extending it is a human gate) — **terrain-emitted:** `water` · `forest` · `open_grass` · `browse` · `flowers` · `sand` · `rocks` · `cultivated` · `snow`; **building-emitted:** `built` · `house` · `large_house` · `barn` · `large_barn` · `stable` · `coop` · `silo` · `mill`; **resident-emitted:** `people` · `deer`. **`quiet` was retired** — no source, no consumer, and a `built` limit does its job better. **`cover` was retired at D-52** once the tier re-spec left it with a source but no consumer; Rock now emits `rocks` alone, consumed by Donkey, Alpaca, Shiba Inu and Stag. `sand` and `coop` remain sourced-or-emitted with no consumer — `sand` has no shipped terrain source, `coop` waits on an unshipped Chicken.
- **Tag-source mapping** — which terrain and which building emits which tag (#5 closed — emission radii and weights are depth, not v1); full tables: [terrain.md](terrain.md) and [buildings.md](buildings.md). **Every placeable emits `built`** in addition to its own tags, so one `built` limit excludes all of them — including buildings added later, without touching a species file. **Rock, not forest, was the `cover` source** — a tag retired at D-52 once nothing consumed it. Historically that made Fox habitat a two-brushstroke composition (forest *near* rock) rather than a side effect of painting forest for Wood; Fox now needs `forest`, `open_grass` and `water`, and rock earns its place through `rocks` (Donkey, Alpaca, Shiba Inu, Stag) — see [roster.md](roster.md).
...
| House (2×2) | ~15 Wood | **Farmhouse** (3×3) ~30 Wood — its own buildable since D-52 |
...
The shared vocabulary every terrain, animal, and ambient system reads from is 20 tags (extended 2026-09-04, with `quiet` and `cover` retired — → D-52; the full grouped list is under Habitat Suitability above). One lesson the pilots surfaced: a fact card can pass every automated check and human read-through and still be factually wrong — only fetching and checking the source catches it — which is why verification is a hard, non-skippable gate (checklist in [spec.md](spec.md)).
...
3. Habitat tagging (each tile emits its own terrain's tags — tags do **not** spread between tiles, → D-25)
...
| 3 | Terraform | Five of six v1 terrains: grass, water, forest, rock, cultivated. Pricing and removal/refund policy ship whole. Cultivated because capacity reads cultivated tiles in radius — row 4's move-in needs it; **rock as the `rocks` source** (#5 resolved) — needed by Donkey, Alpaca, Shiba Inu and Stag. (`cover`, its other emission, was retired at D-52.) |
| 4 | Build | House at 2×2, grass only (→ D-60); the large-house role is **Farmhouse** at 3×3, its own buildable (D-52), and is what a villager *family* gates on. **A villager moves in when its habitat is met** ships whole — the USP requires the proof, not the building. |
