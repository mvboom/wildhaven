...
- **Habitat tag vocabulary** (extended 2026-09-04; extending it is a human gate) — **terrain-emitted:** `water` · `forest` · `open_grass` · `browse` · `cover` · `flowers` · `sand` · `rocks` · `cultivated` · `snow`; **building-emitted:** `built` · `house` · `large_house` · `barn` · `large_barn` · `stable` · `coop` · `silo` · `mill`; **resident-emitted:** `people` · `deer`. **`quiet` was retired** — no source, no consumer, and a `built` limit does its job better. `sand`, `cover` and `coop` currently have no consuming species: `sand` has no shipped terrain source, `coop` waits on an unshipped Chicken, and **`cover` lost its last consumer at D-52** when Fox and Rabbit moved to tiers. Rock still earns its place via `rocks`.
- **Tag-source mapping** — which terrain and which building emits which tag (#5 closed — emission radii and weights are depth, not v1); full tables: [terrain.md](terrain.md) and [buildings.md](buildings.md). **Every placeable emits `built`** in addition to its own tags, so one `built` limit excludes all of them — including buildings added later, without touching a species file. **Rock, not forest, is the `cover` source** — but since D-52 **no species consumes `cover`**, so this fixes the tag's definition rather than any species' habitat. Historically it made Fox habitat a two-brushstroke composition (forest *near* rock) rather than a side effect of painting forest for Wood; Fox now needs `forest`, `open_grass` and `water`, and rock earns its place through `rocks` (Donkey, Alpaca, Shiba Inu, Stag) — see [roster.md](roster.md).
