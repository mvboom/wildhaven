...
| `habitat_needs` | **superseded by `tiers` for capacity — but NOT inert.** `RoamRegion` reads it live as the OR-predicate for which tiles a resident may walk (see [roaming.md](roaming.md)), so it is now the species' *roam-tag* list and is authored on all 15 species. It may legitimately diverge from `tiers`: `fox.tres` carries `["forest"]` here against tiers of `forest`/`open_grass`/`water`. Changing it changes where animals wander |
...
| `footprint` | tile footprint. **Ruled 2026-09-08 (→ D-60):** Chicken Coop and Well 1×1; House, Small Barn, Open Barn, Silo, Windmill, Water Tower 2×2; Farmhouse and Large Barn 3×3. A building emits its tags once, at its centre tile — see the tag model above |
...
| `hotbar_category` | groups this buildable behind one palette button. The eight farm buildings all carry `"farm_building"`; an empty value makes a buildable its own single-member group, which is how House renders alone. One button per **group**, not per definition — see Screen Layouts |
| `model_scenes`, `fact_text` | model look variant(s) + flavor copy — an `Array[PackedScene]` (2026-08-26, building-variety B1). **No `pick_variant()`**, unlike `TerrainDefinition`/`AnimalDefinition`: every placed instance of a buildable shows the same look, so placement reads index 0; which member of a group sits there is the style picker's choice, re-resolved at tap time |
...
**WorldPreset** — one per New Game card. **Three ship:** `meadow_start` (the default), `barren_start`, `forested_start`.

| Field | Meaning |
|---|---|
| `id`, `display_name` | identity — "Meadow", "Barren", "Forested" |
| `width`, `depth` | starting grid size; all three ship 36×36 |
| `base_terrain_id` | what a tile is when `terrain_mix` is empty. **Must stay tag-inert** (`test_world_preset.gd`) — it is `wild_grass` on all three |
| `terrain_mix` | terrain id → relative weight, normalised by `TerrainScatter.quotas()` and placed in **clumps** by a seeded noise field, so a 10% water share arrives as a few ponds rather than scattered single tiles. **Empty is load-bearing, not unfinished:** `barren_start` ships empty and is byte-identical to every pre-D-53 world. Applied only on a real `"new"` intent |
...
- **The tag model (v1) — tags are a property of the tile, not a field radiating from it (→ D-25).** A tile's tag set is a pure function of what occupies it: its terrain type, or its building's `emitted_tags` where a footprint suppresses the ground. **A multi-tile building emits once, at its centre tile (→ D-60)** — its other footprint tiles are occupied and terrain-suppressed but emit nothing, so a building contributes to a count exactly once regardless of footprint. Centre rather than origin because the emitting tile is what a radius is measured to; integer division picks the true centre at odd sizes and the origin at even ones (2×2 has no centre tile; the residual is 0.71 tiles against radii of 5–16). **Tags do not spread to neighbouring tiles**, carry no per-source emission radius, and have no distance weighting. `count_t` in the capacity formula is therefore a plain count: *the number of tiles within the species' radius whose own terrain (or building) emits `t`*. There is no "counts as met" threshold per tile — a tile either emits a tag or it does not.
...
- **The inert-land invariant.** Untouched revealed land must never satisfy any species on its own — nor raise any existing neighborhood's carrying capacity — or pushing the mist becomes a reward and animals arrive (or multiply) on land the player never made, breaking both the mist pillar and the rule that every resident is something the player attracted. **v1 enforces this structurally:** untouched revealed land is *wild grass*, which emits no tags at all (see World Structure in [gdd.md](gdd.md)), so the set of tags untouched land emits is empty by construction, and the invariant covers qualification *and* capacity in one stroke, because both are functions of tag counts and wild grass contributes zero to either. **Two things about the shipped enforcement are worth stating plainly.** First, `TerrainDefinition.derive_bare_tags()` does return empty over real data, but `AnimalDefinition.BARE_TAGS` is hardcoded to `["open_grass"]` — a deliberate strict **superset** of the derivation, so the species-side subset check is over-enforced rather than under-enforced, which is the safe direction to err for a pillar invariant. Making that constant read the derivation is open work, tracked by `test_bare_tags_derivation.gd`. Second, **`WorldPreset.terrain_mix` is a deliberate, scoped exception (→ D-53)**: a Meadow or Forested start hands the player live habitat on frame one, because "choose a starting land" is meaningless if every choice builds the same inert grass. The invariant still holds everywhere it was written for — revealed mist land, `wild_grass` itself, and every preset's `base_terrain_id`. The automated check in the `.tres` validation suite remains, guarding the construction itself: it derives `BARE_TAGS` **from the tag-source mapping at validation time, never hardcoded** — otherwise the check silently rots the first time tag emission changes — and fails the build if any source ever gives untouched land a tag; the qualitative subset check (**no species' `habitat_needs` ⊆ `BARE_TAGS`**) stays as the backstop for that future. The Add-an-Animal pipeline inherits the gate for free.
- **News Report content** is a per-animal (or general) text pool reusing the fact-card pipeline. **The first general entry ships 2026-09-08 (→ D-63):** `NewsReportContent.NO_FOREST_REPORT`, shown when the world holds zero Forest tiles **and the stockpile is below `LOW_WOOD_FLOOR` (30 Wood)** — the one state in v1 where the economy is genuinely stalled (no Forest, no Wood, ever) and the HUD cannot say why. The wood gate is what keeps a *new* world's feed about villagers and animals: 100 Wood is several builds, so the player is solvent, not stuck, and the branch holds itself shut with no new-game flag or timer. It **alternates** with the species hints, reusing the presenter's existing no-repeat latch via a pseudo-id that matches no roster species, so it never becomes the repeated nag §10.1 rules out.
- **Save file** is JSON with a `save_version` field — now **7** (bumped for D-57's captured styles, which added `tile_styles` and a per-building `style`; both are additive and both have a correct meaning when absent, so the v6→v7 step is pure version-bump bookkeeping. v6 had added a `count` to pending-arrival entries for group arrivals; a missing `count` restores as 1, never 0). `migrate()` chains every step from v1 and is idempotent. Full contents under Technical Overview → Saves in [gdd.md](gdd.md).
...
```
┌────────────────────────────────────────────────┐
│ [🪵 Wood: 240]                                  │   top-left: resource + population counters (D-34)
│ [Species Hosted: 4]                             │
│ [Currently Resident: 3]                         │
│ [Village Population: 6]                         │
│                 (world view)                    │
│                                                 │
│ [?]   [0 Info][1 Grass][2 Water]…[Erase]  [↺][↻]│   bottom-left: Help · bottom-centre: the palette row
└────────────────────────────────────────────────┘    bottom-right: Rotate ×2, beside Exit
```

- **There is no three-button mode switch, and there is no per-mode palette.** The palette row across the bottom centre is **always visible, in every mode**, and pressing a button sets the mode implicitly: a terrain entry selects Terraform, a placeable entry selects Build, `Info` returns to Inspect. `GameHud.Mode` still has exactly three members and `TapRouter` still resolves exactly one gesture — what changed is that picking the tool and picking the mode became one press instead of two. The earlier 5-slot drag-fill hotbar and the browse grids that fed it are both retired.
- **The row reads left to right as** `Info` (key `0`) — every catalog entry (keys `1`–`9`) — `Erase`. Info and Erase are the two fixed tools flanking the catalog; neither is content, and neither appears in `palette_option_ids()`.
- **It is data-driven, but grouped.** One button per *group*, not per definition: the eight farm buildings collapse behind **Farm Building** (`PlaceableDefinition.hotbar_category`), and `grass`/`wild_grass`/`meadow`/`scrub` collapse behind one **grass-family** button (a hardcoded id list, since `TerrainDefinition` carries no category field). `snowfield` is deliberately excluded and stands alone. That is 6 terrain buttons + House + Farm Building = **8**, the count the band between the corner clusters was sized for. A grouped button's label and icon follow the current pick.
- **Long-press opens the style picker** for a grouped button (grass family, forest, House, Farm Building) — an accepted optional layer, never required; see Player Interface & Controls in [gdd.md](gdd.md).
- **Top-left counters** (Wood, Species Hosted, Currently Resident, Village Population) are read-only indicators (Pillar 1), stacked at all times, at every zoom level and during the Home-key map peek alike. Never flash, never demand attention when low — the four setters assign a string and do nothing else: no tween, no colour change, no low-balance state, no threshold anywhere. Species Hosted is a bare running count with no denominator (D-34).
- **Bottom-right is the two Rotate buttons** (D-44), locked beside the Exit button. **There is no Settings gear and no on-screen Home button** — Settings left the world view entirely (D-50), and the terraform-bar rework retired the redundant Home button; the Home key still performs the map peek.
- **The Field Guide has no top-right icon.** It is a tab of the browse shell, opened with Tab or the bottom-left `[?]` button.
...
- **The displacement warning is shown for player-caused edits only (amended 2026-09-07).** `GentleDisplacement` opens a settlement gesture from two places: a player edit (`on_edit()`) and an animal landing (`on_arrival()`, which rebuilds the tile-exclusivity map and can push a neighbouring home under its population). Only the first is gdd.md's "the player's own settled choice"; the second is the game's own initiative and fires continuously in a settled world. `SettlementWindow` carries a `player_caused` flag (merged by OR, so an edit joining an arrival-armed gesture still warns), it rides in the `displacement_warned` payload, and `GameUI` gates the warning panel **and both consequence banners** on it. The simulation is unchanged — every displacement still runs and still emits its signal.
- The **Read-Aloud button** (🔊) is present on the fact card — floor and full alike, and on both of the card's entry points (a species' first-ever arrival, and an Inspect tap) — **and on the displacement warning panel** (`DisplacementNotice`), because consent must not require fluent reading. It is **not** on the top-centre toasts. **Cards also speak themselves on open** (`FactCard.AUTO_SPEAK`, closing #13 → D-29), gated by a speaking toggle that lives on the Title screen; every entry point routes through the same `ReadAloud.speak()`, which degrades silently on a machine with no voice — the button hides rather than offering a control that does nothing. Wider coverage (News Reports, Field Guide) is deferred (future.md).
  **Amended 2026-09-07 — the right-side rolling feed is deleted.** It carried repeat fact cards and displacement warnings; it churned continuously because `HabitatSimulation._move_in()` emits `resident_arrived` once per **individual** and a neighbourhood fills one at a time (each landing re-marks it dirty, enqueuing the next on `ArrivalQueue`'s 20–60 s delay), so N settled sites announce something every (20–60)/N seconds indefinitely. **A repeat arrival now shows nothing at all** — the animal appearing, the row-11 counters and the Field Guide are the whole notification. Displacement warnings returned to `DisplacementNotice.show_warning()`, the centred non-blocking consent panel; the Inspect-tap replay returned to the fact card, where blocking is fine because the player asked for it.
...
| Passive Wood rate | ~1 Wood / forest tile / 60 s, **throttled to 1 Wood / 60 s total above 1000 Wood — ruled 2026-09-08 (→ D-63)**. A cliff, not a taper; passive accrual only, so refunds and the future tap-to-tend burst still pay in full. The *unthrottled* rate is still #8's | balancing (#8); the cap is CLOSED (D-63) |
| House cost | House ~15 Wood; **Farmhouse** ~30 (its own buildable since D-52, not a House form). Costs are UNCHANGED by D-60's footprint re-cut, so wood-per-tile-claimed fell — deliberate; #26 answered that on the stockpile side 2026-09-08 (→ D-62, 50 → 100 Wood), and the costs themselves are still #8's to settle | balancing (#8, #26) |
...
| 4 | Build | The placement-validation family beyond the floor. The footprint ladder itself is no longer depth — it landed 2026-09-08 (→ D-60), floor House included. The Shed is still a deferred 1×1 placeable (future.md), but in its own right, not as a retained asset |
...
|---|---|---|
| 4 | **Final starter roster** — *narrowed:* the roster has **no target count** (→ D-24); it is a floor of three plus depth drawn from a nine-species cleared pool. **Fox and Rabbit shipped** (pilots 3, 3b); Human's fact card closed 2026-07-28 (→ #31, D-28). **Human's asset audit is now reconciled and CLOSED:** [content-pipeline-status.md](content-pipeline-status.md) records `pre_import_audit: done` — 5 original variants plus a 13-model character-pack batch, CC0 confirmed per pack, animations verified per model rather than assumed, with `Male_Suit`/`Smooth_Male_Suit` held out against art.md's no-pure-black rule. **`human.tres` wires all 18.** Step 3 for the cleared pool closed at D-43. Open: fact copy for two pool species (Bull's is provisional, Shiba Inu has none). Audit finding governing the rest: **animation, not species availability, is the scarce resource** (of 37 rabbit models across two CC0 libraries, exactly one was rigged and animated) | copy for Bull and Shiba Inu |
...
| 10 | **World preset list** — **CLOSED 2026-09-07 (→ D-53).** Three presets ship as selectable New Game cards, each a `WorldPreset` resource: `meadow_start` (default), `barren_start`, `forested_start`. The second half — whether the cards build genuinely different worlds — is what D-53 answered: each carries a `terrain_mix` that `TerrainScatter` clumps from the world seed, and Barren keeps an empty mix deliberately. Adding a fourth preset is authoring one more `.tres` | closed |
...
| 13 | **Read-Aloud default state** — **CLOSED 2026-08-01 (→ D-29): on.** `FactCard.AUTO_SPEAK = true`, so a card speaks itself when it opens; the player turns it off with the speaking toggle on the Title screen. Still open, and not this row's: whether Master Volume should also silence Read-Aloud | closed |
...
| 18 | **World dimensions** — start size (~36×36) and cap (~128×128). **The footprint half is CLOSED (2026-09-08, → D-60):** every buildable's footprint was ruled against a rendered scale comparison — see buildings.md's table. What is still open here is the world's own size, plus a human eye pass on building facing, which no footprint ruling touched | GDD refinement pass |
...
| 26 | **Starting Wood stockpile** — *inverted 2026-09-08 (→ D-62).* The **exact value is now decided: 100 Wood** (`WoodLedger.STARTING_WOOD`), raised from the ~50 placeholder because D-60 grew footprints without moving costs. What is open is the half that used to be settled: 100 covers several builds, not the nudge's first build only, so the **sizing principle** — not a buffer, pacing beginning at the second build — no longer describes the shipped value and awaits a human restatement | balancing/playtesting |
